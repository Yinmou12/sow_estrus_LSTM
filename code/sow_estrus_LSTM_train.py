...
from lstm_model import EstrusLSTM
...
def evaluate_model(model, loader, criterion, device):
    ...
    with torch.no_grad():  # 评估时不计算梯度，节省内存和计算资源
        for batch_X, batch_y in loader:
            ...
            preds = (outputs >= 0.5).float().cpu().numpy()  # 将概率转换为二分类标签
...
def main():
    ...
    train_loader = DataLoader(
        EstrusDataset(X_train, y_train), batch_size=32, shuffle=True
    )
    ...
    model = EstrusLSTM(input_size=1, hidden_size=64).to(device)
    ...
    _, t_acc, t_precision, t_recall, t_f1, t_auc = evaluate_model(
        model,
        DataLoader(EstrusDataset(X_test, y_test), batch_size=32),
        criterion,
        device,
    )
